...
export const useArrayWheeler = <T,>(data: T[], options?: Partial<WheelerProps<T>> & { all?: string }) => {
    const { all } = options ?? {}
    ...
    return {
        ...
        renderer: [r => r],
        valuer: [r => r],
        checkboxer: all ? [r => checked[data.indexOf(r)]] : null,
        checkbox: [$(true)],
        noMask: true,
        hideOnBackdrop: true,
        rows: Math.min(6, data.length),
        open: $(false),
        ...options ?? {}
    }
}
